...
import { createPaymentQuote, loadPrimaryAssets, sendPaymentQuote } from "@/lib/fyora/particle";
...
export function PaymentSheet({
  ...
  const { identity, refreshIdentity, signInWithEmail, signInWithGoogle } = useMagic();
  ...
  const handleEmailLogin = async () => {
    setConnecting(true);
    try {
      const next = await signInWithEmail(email);
      await loadBalance(next.evmAddress);
      setStep("balance");
    } catch (error) {
  ...
  const handleGoogleLogin = async () => {
    sessionStorage.setItem("fyora-auth-return", window.location.href);
