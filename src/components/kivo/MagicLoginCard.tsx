...
export function MagicLoginCard({ title = "Sign in to Fyora" }: { title?: string }) {
  const [email, setEmail] = useState("");
  const [working, setWorking] = useState(false);
  const { signInWithEmail, signInWithGoogle } = useMagic();
  ...
  return (
    <div className="mx-auto max-w-md rounded-3xl bg-card chunky-thick shadow-sticker-lg p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Email or Google. No wallet extension.</p>
      </div>
