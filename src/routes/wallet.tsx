...
function WalletCenter() {
  ...
  const tokenPrice =
    assetsQuery.data?.assets.find((asset) => tokenKey(asset.tokenType).includes(tokenId))?.price ??
  ...
  const submitTransfer = async () => {
    ...
    try {
      ...
      const result = await sendPaymentQuote(
        quote.account as Parameters<typeof sendPaymentQuote>[0],
        quote.transaction,
      );
      setStage("submitting");
    ...
    return (
      <div className="min-h-screen bg-paper text-ink">
        ...
        <div className="px-4 py-16">
          <MagicLoginCard title="Sign in to open your wallet" />
        </div>
      </div>
    );
  }
  ...
  const receiveAddress =
    receiveNetwork === "solana" ? (identity.solanaAddress ?? "") : identity.evmAddress;
  const feeUsd = quote
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">
              One Universal Balance across supported chains, with Magic securing every signature.
            </p>
          </div>
          <div className="inline-flex items-center gap-2 rounded-full bg-card chunky shadow-sticker-sm px-4 py-2 text-sm font-semibold">
            <ShieldCheck className="h-4 w-4" /> Powered by Magic
          </div>
        </div>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-[1.4fr_1fr]">
          <div className="rounded-3xl bg-ink p-6 text-paper chunky-thick shadow-sticker-lg sm:p-8">
            ...
            <div className="mt-3 font-display text-5xl italic sm:text-7xl">
              {assetsQuery.isLoading ? "..." : money(assetsQuery.data?.totalAmountInUSD ?? 0)}
            </div>
            ...
            <button
              type="button"
              onClick={() => Promise.all([assetsQuery.refetch(), activityQuery.refetch()])}
          ...
          <div className="rounded-3xl bg-card p-6 chunky-thick shadow-sticker-lg">
            ...
            <div className="mt-4 inline-flex rounded-full bg-secondary p-1 chunky">
              {(["evm", "solana"] as const).map((network) => (
                <button
                  key={network}
                  type="button"
            ...
            {receiveAddress ? (
              <div className="mt-4 flex items-center gap-4">
                <div className="rounded-xl bg-white p-2 chunky shadow-sticker-sm">
                  <QRCodeSVG value={receiveAddress} size={104} fgColor="#141313" />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="break-all font-mono text-xs">{shortAddress(receiveAddress)}</div>
                  <CopyButton value={receiveAddress} label="Copy address" className="mt-3" />
                </div>
              </div>
            ) : (
              <p className="mt-4 text-sm text-muted-foreground">
                Magic has not provisioned a Solana address for this account.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
          <div className="rounded-3xl bg-card p-6 chunky-thick shadow-sticker-lg sm:p-8">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Particle selects source liquidity across your Universal Balance.
            </p>
...
function TransferStatus({ stage, universalXUrl }: { stage: TransferStage; universalXUrl: string }) {
  ...
  const label: Record<TransferStage, string> = {
    ...
    quoted: "Quote ready",
    signing: "Waiting for Magic confirmation",
